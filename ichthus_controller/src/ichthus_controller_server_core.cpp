...
#define SET_STATE_VAR(var,val)	\
  ...
  string state_var_list[MAX_STATE_VARS] = {
    ...
    "vehicle_steering_angle_can",
    "cc_target_velocity",
    "sc_target_anglvelo",
    "sc_target_angle",
    "velocity_update", //20200121
    ""
  };
  ...
  double adjust_value_to_range(state_var& var, double val) {
    ...
    if (val < var.lower) {
      cout << "set: value(" << val << ") adjusted to lower limit(" << var.upper << ")!!!" << endl;
      val = var.lower;
    }
  ...
  bool __sethand_motor_pos(state_var& var, double val, int motor_id) {
    ...
    if(var.lower < 0)
      val = var.value / var.step;  //20200121 steer wheel 계산 수정
    else
  ...
  bool sethand_velocity_update(state_var& var, double val){
    ...
    var.value = val;
    return true;
    //20200121 state_var 와 일반변수 결정 필요
  }

  //즉시 standby 상태로 가게 되어, state 의 완료 상태를 확인할 수 없어, motion_state 1 로 가지 않도록 주석처리를 하였습니다.
  ...
  bool normalize_position_variables(const string& vpos_var_name, const string& pos_var_name) {
    ...
    if(vpos_var.lower < 0)
      vpos_var.origin = vrange * (pos_var.upper + pos_var.lower) / range; //20200121 steer wheel 계산 수정
    else
  ...
  int can_steering_error=-200;
  
  void IchthusController::callback_CAN(const ichthus_controller::can_msgConstPtr& msg){
    receiving_from_can_gateway = true;
    if(update_vehicle_velocity)
      set_state_var("vehicle_velocity_can",msg->velocity);
    set_state_var("vehicle_steering_angle_can",msg->steering_angle+can_steering_error);
  }
  ...
  void IchthusController::callback_vehicle_cmd(const autoware_msgs::VehicleCmdConstPtr& msg){
    set_state_var("cc_target_velocity",msg->ctrl_cmd.linear_velocity * 3.6);
    set_state_var("sc_target_angle", msg->ctrl_cmd.steering_angle);
  }
  ...
  void IchthusController::doLoop() {
    ...
    load_state_vars();
    //print_state_vars();
    ros::Subscriber can_sub = nh.subscribe("/vehicle_status_can",100, &IchthusController::callback_CAN, this);
    ros::Subscriber obd_sub = nh.subscribe("/vehicle_status_obd",100, &IchthusController::callback_OBD, this);
    //ros::Subscriber twist_filter_sub = nh.subscribe("/twist_cmd",100, &IchthusController::callback_twist_cmd,this);
    ros::Subscriber vehicle_cmd_sub = nh.subscribe("/vehicle_cmd",100, &IchthusController::callback_vehicle_cmd,this);
