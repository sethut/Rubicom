...
#define MAX_STEER_WHEEL_ANGLE_FROM_CAN  4680
  ...
  bool does_pedals_reach_goal(void) {
    return (does_motor_var_reach_goal(motor_id_pedal_decel)
	    && does_motor_var_reach_goal(motor_id_pedal_accel));
  }
  ...
  void move_pedal_decel(double vpos) { // vpos should be positive!!!
    string name;
    double origin;
    homing_pedal_accel();
    name = motor_id_to_vpos_name(motor_id_pedal_decel);
  ...
  void calibrate_steer_wheel(void) {
    state_var var;
    string name;
    ...
    get_state_var_value("vehicle_steering_angle_can",can_angle_value);
    if (receiving_from_can_gateway) 
  ...
  void move_steer_wheel(double vpos) {
    ...
    if (receiving_from_can_gateway)
      ...
      set_state_var(name, vpos);
    cout<<"<debug> steer vpos : " <<vpos<<endl;
  }
  
  enum gear_stick_level { GS_PARK, GS_DRIVE };
  ...
  double kmph_to_mps(double velocity) {
    return velocity / 3.6;
  }
  ...
  bool pull_over(void) { 
    ...
    if (state == 0) { // initial state 
      ...
      pull_over_ini_vel = kmph_to_mps(pull_over_ini_vel);
    }
  ...
  bool ready_to_drive(void) {
    ...
    if (state == 0) { // initial state
      ...
      if(does_pedal_accel_reach_lower() && does_pedal_decel_reach_upper()){
        state = 2;
	      calibrate_steer_wheel();
      }
      return false;
    }

    else if (state == 1) { // emergency-stop in progress 
      if (!(does_pedal_accel_reach_lower() && does_pedal_decel_reach_upper()))
	      return false;
      state = 2; 
      calibrate_steer_wheel();
      ...
      if (does_steer_wheel_reach_goal()) {
        cout<<"state == 1 calibrating finished"<<endl;
	      state = 3;
    ...
    else if (state == 2) { // steer-wheel-calibrating in progress
      if (!does_steer_wheel_reach_goal())
	      return false;
      state = 3;
      move_gear_stick_to_drive();
    ...
    else if (state == 3) { // gear-stick-moving in progress
      if (!does_gear_stick_reach_goal())
	      return false;
      state = 0;
      return true;
    ...
    cout << "ready_to_drive : unknown internal state!!!" << endl;
    return false;
  }

  enum pedal_type { PEDAL_ACCEL, PEDAL_DECEL };
  int pedal_selected = PEDAL_ACCEL;
  int prev_velo_timestamp = 0;
  ...
  double estimate_pedal_position(double actual_velocity) {
    ...
    if (cc_target_velocity != target_velocity) { // target velocity updated!
      cc_target_velocity = target_velocity;
      tvelo_updated = true;
      pedal_persist = true;
      reference_velocity = actual_velocity;
      decel_needed = (actual_velocity > target_velocity ? true : false); //20200126
    ...
    else { // tvelo_updated == true
      if (decel_needed) {
	      pedal_selected = PEDAL_DECEL;
	      reference_velocity = cc_target_velocity;
	      tvelo_updated = false;
      }
      else { // decel_needed == false
	      pedal_selected = PEDAL_ACCEL;
    ...
    case PEDAL_ACCEL:
      ...
      if (pedal_switched) { // DECEL -> ACCEL (e.g., downhill -> uphill)
	      if (actual_velocity >= reference_velocity) { // keep accu_ref_err
	        pedal_switched = false;
	        num_latest_velo = 0;
	      }
	      else { 
	        ...
	        ++num_latest_velo;
	        if (num_latest_velo > 3) { // if 5ms, it will be num_latest_velo > 6
            cout<<"reference > actual more than three times"<<endl; // cannot catch up reference_velocity so far
	          if (latest_velocities[0] >= latest_velocities[3]) { // boost accu_ref_err (mostly for obd)
	            double base, step;
      ...
      if (actual_velocity <= reference_velocity + p_margin) { // reference tracking!
	      pedal_persist = false;
	      ...
	      if (pedal_persist) {
	        ...
	        else if (curr_tgt_err > prev_tgt_err) { // vehicle is getting even faster
    ...
    case PEDAL_DECEL:
      ...
      if (actual_velocity >= reference_velocity - n_margin) { // reference tracking!
	      pedal_persist = false;
	      converge_towards_ref = 0;
      }
      else { // far from reference!
	      if (pedal_persist) { // note that the following errors are negative!
	        if (curr_tgt_err > prev_tgt_err) // vehicle is getting faster //error is going to bigger
	          ++converge_towards_ref;
	        else if (curr_tgt_err < prev_tgt_err) { // vehicle is getting even slower //error is going to smaller
	          if (!converge_towards_ref) 
              --converge_towards_ref;
	        }
  ...
  bool cruise_control(void) {
    ...
    else { // (pedal_selected == PEDAL_DECEL)
  ...
  double convertAngularVelocitytoSteeringAngle(void) {
    double omega;
    double v;
    double radius;
    double wheel_base;
  ...
  bool steer_control(void) {
    ...
    get_state_var_value("sc_target_angle",steering_angle);
    //steering_angle = convertAngularVelocitytoSteeringAngle();
  ...
  bool homing_trial=false;
  bool estop_trial=false;
  bool calibrate_trial=false;

  void run_controller(void) { // called when ecat_state == 2.
    double value;
    int motion_state;
   
    get_state_var_value("motion_state", value);
    
    motion_state = value; // type-cast is necessary for integer comparison!!!
#if 0
    cout << "motion_state = " << motion_state << endl;
    set_state_var("motion_state", 1);
    motion_in_progress = false;
#endif
    
    if (motion_state == 0) {  // initial state
      //cout << "motion: initial state" << endl; // do nothing
      motion_in_progress = false;
    }

    else if (motion_state == 1) { // standby state
      //cout << "motion: standby state" << endl;
      stand_by(); 
      motion_in_progress = false;
      // reset_pid_values_for_cc()
    }

    else if (motion_state == 2) { // pedal-homing state
      //cout << "motion: pedal-homing state" << endl;
      if(homing_trial == false){
        homing_pedals();
        homing_trial=true;
      }
      else{
        if(does_pedal_accel_reach_lower()&&does_pedal_decel_reach_lower()){
          motion_in_progress = false;
          homing_trial=false;
          set_state_var("motion_state", 1); 
        }
    ...
    else if (motion_state == 3) { // estop state
      //cout << "motion: estop state" << endl;
      if(estop_trial==false){
        emergency_stop();
        estop_trial=true;
      }
      else{
        if(does_pedal_decel_reach_upper()&&does_pedal_accel_reach_lower()) {
          motion_in_progress = false;
          estop_trial=false;
          set_state_var("motion_state", 1);
        }
    ...
    else if (motion_state == 4) { // steer-wheel-calibrating state
      //cout << "motion: steer-wheel-calibrating state" << endl;
      if(calibrate_trial==false){
        calibrate_steer_wheel();
        calibrate_trial=true;
      }
      else{
        if(does_steer_wheel_reach_goal()){
          motion_in_progress = false;
          calibrate_trial=false;
          set_state_var("motion_state", 1);
        }
      }
    }
    ...
    else if (motion_state == 5) { // pull-over state
      //cout << "motion: pull-over state" << endl;
      if (pull_over()) {
        if(does_pedal_decel_reach_upper()&&does_pedal_accel_reach_lower()){
	        motion_in_progress = false;
          set_state_var("motion_state", 1);
    ...
    else if (motion_state == 6) { // ready-to-drive state
      //cout << "motion: ready-to-drive state" << endl;
      if (ready_to_drive()) {
        if(does_pedal_accel_reach_lower()&&does_pedal_decel_reach_upper()&&does_steer_wheel_reach_goal()&&does_gear_stick_reach_goal()){
	        motion_in_progress = false;
          set_state_var("motion_state", 1);
        }
      }
    ...
    else if (motion_state == 7) { // cruise-control state
      //cout << "motion: cruise-control state" << endl;
        if(cruise_control()){
          motion_in_progress = false;
    ...
    else if (motion_state == 8) { // automated-driving state
      //cout << "motion: automated-driving state" << endl;
    ...
    else if (motion_state == 9) { // avc-driving state
      //cout << "motion: avc-driving state" << endl;
        double vel; 
